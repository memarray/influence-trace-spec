...
This is a spec, not a product announcement. The reference implementation (MemArray) is in development. Publishing the spec first is meant to nail down the primitives in public so any memory system (open source or commercial) can adopt them, argue with them, or extend them.
...
This is not a knock on Mem0 specifically. Zep ships bi-temporal timestamps and a temporal knowledge graph (Graphiti), which is excellent for "when was this true?", but it still does not expose rollback, response-level attribution, or version diffs as first-class primitives. Letta's `Context Repositories` (Feb 2026) introduce git-style memory snapshots for coding agents, which is the closest analog in the field, but the model is repository-scoped rather than fact-scoped and does not include response-level attribution.

The audit trail, in other words, is a solved problem. The *operational layer above the audit trail* (roll back, time-travel, diff, trace) is not.
...
The voice-agent ecosystem is converging on a small set of platforms (LiveKit Agents, Vapi, Retell), and as of April 2026 none of them ship first-party persistent memory. All three direct users to third-party memory layers. So the developer experience for voice memory is: pick Mem0/Zep/Supermemory, glue it in, hope it doesn't overwrite the wrong thing, and reconstruct what happened after the fact when something goes wrong.

The primitives in this spec are general; they are not voice-specific. The design constraints (sub-300ms retrieval, op-log writes off the hot path, traces written asynchronously) are calibrated for voice because voice is the most demanding case. A memory system that fits inside a voice turn fits inside any other agent loop too.
...
A **fact** is the smallest piece of memory the system stores: a typed, structured record, not a raw text chunk. The intent is to make memory inspectable. A fact is something you can look at, edit, invalidate, or roll back, not a paragraph you have to parse.
...
- **Source reference:** where this fact came from: conversation ID, message ID, timestamp, speaker.
...
The `op_log` is the spine of the system. Every mutation (insert, supersession, invalidation, merge, rollback) writes one row to `op_log`. The schema is:
...
And, for each fact the retrieval returned, a **response-to-fact link**:
...
A response trace is the answer to "which memories caused this answer?" It is written off the hot path (the trace insert does not block the agent's response), but it is durable, indexed by `response_id`, and exposed via `GET /v1/trace/{response_id}`.
...
where `r̂` is `r` rescaled to `[0, 1]` via `r̂ = r / (1 / (k_const + 1))`, and the default weights are `w_p = 1.0`, `w_r = 0.4`, `w_s = 0.6`. The leading `w_p · p` factor means: if the fact was not in the prompt, its grounding score is zero. That is deliberate. A fact the LLM did not see did not ground the response, and the score should say so.
...
The honest causal version is: rerun the LLM with the fact removed and observe whether the response changes. The system supports that via `POST /v1/trace/{response_id}/counterfactual`, which re-runs the response with a specified fact omitted from the prompt and returns the new completion alongside the original. That is too expensive to run on every response, but cheap enough for the handful a developer is actually investigating.
...
- **Rollback of a write that was the basis for a later response.** A response trace that depended on a fact that has since been rolled back is annotated, on read, with `rollback_affected: true`. The trace itself is never modified. Traces stay as historical records of what the system actually did, even after undo.
...
**Why both axes matter.** Compliance and debugging often mean asking what the system believed at decision time (`as_of = decision_time`, `known_by = decision_time`). Another common question is what was actually true at decision time given everything known now (`as_of = decision_time`, `known_by = now()`). Those produce different answers. A memory system that collapses them into one timestamp cannot tell them apart.
...
**Behaviour.** Given a `memory_id` (which here means a `subject` plus a `predicate`, e.g. `("alice", "dietary_preference")`), and two version anchors, return a structured diff of the fact's state between those anchors. Version anchors can be:
...
**Why structured.** A text diff of two fact JSON blobs would be readable to humans but awkward for code. Compliance tooling, audit dashboards, and downstream alerting want to query things like whether the `value` field changed or whether `confidence` dropped more than 0.2 in one op. That needs a structured diff, not a blob of unified diff text.
...
The trace shows the patient's name fact was *not* retrieved at all; it had been superseded yesterday. One more call:
...
**With this spec.** For each failing eval case, pull the response trace and look at `in_prompt` and `grounding`. If facts are retrieved (high similarity, good rank) but `in_prompt = false`, the bug is usually prompt truncation: the new preamble eats budget and facts drop out. If facts are `in_prompt = true` with high `grounding` but the model still ignores them, the bug is usually instruction style: the facts are in the prompt but the model is told to weight them oddly.
...
The architecture fits on three commodity servers: one for the API + embedder + workers, one as a hot standby plus benchmark runner, one for Postgres + Redis. That is deliberate. The reference implementation should be runnable by a small team without a dedicated platform org.
...
A 25-40% cache hit rate on realistic voice workloads (measured on prototypes, not yet benchmarked publicly) brings the effective P95 well under 150ms.
...
This spec is published as a request for comment, not a finished standard. It will change with implementation experience, community feedback, and PRs against the reference implementation. The hope is that any memory system, open source or commercial, can adopt the primitives, push back on them, or extend them.
...
**Contributing:** Issues and PRs welcome at `github.com/memarray/influence-trace-spec`. Substantive disagreements (e.g. "this primitive is wrong" or "this signal in the grounding score is misweighted") are the feedback that actually moves the document.
