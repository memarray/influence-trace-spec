...
2. **Operationally reversible memory.** Every mutation writes a typed entry to an append-only op log. The system exposes `rollback(op_id)`, `query_at(timestamp)`, and `diff(memory_id, v1, v2)` — primitives today's memory vendors don't ship.
